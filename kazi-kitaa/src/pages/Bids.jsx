...
export default function JobDescriptionScreen() {
  ...
  useEffect(() => {
    const fetchApplications = async () => {
      try {
        setIsLoading(true)
        fetch('http://localhost:2000/'+ `api/jobs/${jobId}/applications`)
          ...
          <div className="feed-section">
            <div className="post-box job-header-card">
              <div className="job-title-section">
                ...
                } }>Job : {job?.description || 'Job Title'}</h1>
                <button 
              ...
              {/* <button className="apply-btn main-action" onClick={handleApply}>
                ...
                {Array.isArray(applications) && !isLoading && !error && applications.map(app => (
                        <div key={app.id} className="post-box">
                            <h2 onClick={
                                (e) => {
                                    e.preventDefault();
                                    navigate(`/profile/${app.applicant.id}`)
                                }
                            }>{app.applicant.name}</h2>
                            <p>{app?.cover_letter || 'No description provided'}</p>
                            <p>Resume <a href={app.resume_url}>Open resume</a></p>
                            <p>Potfolio <a href={app.portfolio_url}>Open a Potfolio</a></p>
